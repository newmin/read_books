...
## 책에서 기억하고 싶은 내용
### Episode 30 : 코로나가 준 레거시 시스템의 교훈
- 현재의 파이썬의 위상을 가졌던 코볼, 지금 이렇게 커다란 역할을 하는 파이썬 또한 언젠가 역사의 뒤안길로 사라진다
  
### Episode 31 : 데이터와 단짝 친구, SQL
- Oracle, MySQL, MariaDB 등은 DBMS를 뜻하는 것으로 이건은 "DB"와는 다르다! DB관리시스템임.
- 관리시스템 마다의 SQL을 사용하는 방식에 약간의 차이가 있음 -> 한국어를 사용하지만, 사투리로 인해 약간의 차이가 있는 정도의 느낌
  
### Episode 32 : NoSQL이 뭐죠?
- 도큐먼트(Document) DB :
  - 데이터 형식이 자유로움
  - JSON 도큐먼트 형태로 저장 -> 열과 행의 개념이 없고, 대괄호와 중괄호로 저장
  -  Ex.) 몸고DB
- 키값(key-value) DB :
  - 읽고 쓰는 속도가 빠름
  - Ex.) 카산드라DB, 다이나모DB
- 그래프(Graph) DB :
  - 노드로 관계 표현
  - 열이나 도큐먼트의 개념 대신, 노드를 필요로 함
  - Ex.) 페이스북의 사용자 간 친구, 좋아요 등의 기록

 -> 무엇이 좋고 나쁨은 없다. SQL과 NoSQL은 엄연히 다른 것이기 때문. 일반적으로는 SQL을 쓰되, 필요하다면 그때 NoSQL로 바꾸어도 무방

### Episode 33 : 깃 & 깃허브, 똑같은 거냐고?
- 깃 : 파일이력 관리 프로그램
- 깃허브 : 파일과 깃으로 관리한 이력을 저장하고 공유하는 공간 -> 클라우드 서비스

### Episode 34 : 버전을 표기하는 방법도 있어요?
- Sem Ver (semantic versioning specification);시맨틱 버저닝 : 숫자 3개로 표현
- 새롭다할 수준의 커다란 업데이트 . 마이너 업데이트 . 패치나 버그 수정 ; 1.7.8 -> 첫번째 업데이트에서, 7번의 마이너 업데이트가 이루어졌고, 이에 대해 8번의 간단한 패치 및 버그 수정이 이루어짐


## 오늘 읽은 소감
- 현재의 파이썬의 위상을 가졌던 코볼, 지금 이렇게 커다란 역할을 하는 파이썬 또한 언젠가 역사의 뒤안길로 사라진다는 것이 인상적
- 현재 우리나라는 JAVA공화국이라 불리우고 있는데, 언젠가 JAVA가 뒤안길로 사라지고 몇몇 방치된 프로그램들을 나중에 쓰게 됐을 때, 비슷한 현상이 일어날 수 있지 않을까? 코볼을 코볼트로 착각했듯, JAVA를 JAVA Script와 착각하고..
- DBMS와 DB를 동일시 했기에 두개가 다르다는 점에서 충격적. 그럼에도 둘은 항상 세트이기에 큰 문제는 없었던걸로 추정
- NoSQL을 사용한 경험이 없어서 이를 활용한 프로젝트를 진행해보고 싶음. 앞서 읽은대로 새로운 프로젝트가 아닌 기존 프로젝트를 활용할 수 있는 방안 모색
...
Episode 36 : 객체 지향 프로그래밍이 뭐죠?1
Episode 37 : 객체 지향 프로그래밍이 뭐죠?2
...
### Episode 35 : 비밀번호는 어떻게 저장될까?
- 해시함수 활용
  - 규칙1 : 동일한 input이면 동일한 output
  - 규칙2 : 입력값 살짝만 바뀌어도 출력값은 커다란 변화
  - 규칙3 : output을 입력한다고해서 input이 나오지 않는다
  > 레인보우 테이블 : 해시함수의 원래값과 변경값을 연결해둔 표 -> 테이블이 털리면 위험
  > 솔트 : 원래값에 무작위 텍스트를 일부 붙임으로써 레인보우 테이블을 보더라도 원래값을 알 수 없도록 함

### Episode 38 : 함수형 프로그래밍이 뭐죠?
- 선언형 프로그래밍 : 결과만을 선언함.
  - Ex.) css: body {background-color : red },  replace(" ","")
- 명령형 프로그래밍 : 과정을 구현함
  - Ex.) for(let i=0;~~){ ~~~~} 구현
- 함수형 프로그래밍 : 명령으로 함수를 구현하고, 해당 함수의 호출을 통해 선언형 프로그래밍 컨셉을 유지


## 오늘 읽은 소감선
- 비밀번호의 암호화와 복호화 과정이 존재함은 알고 있었으나, 계정이나 권한 테스트를 위해 임의계정 접속시에 암호화된 비밀번호과 왕창 바뀌는걸 보며 마냥 신기했던 것이 이러한 원리라는 걸 알게 되어 기쁨
- 함수형 프로그래밍의 구체적인 정의를 알게 됨
- 흔히 듣고 보던 것들에 대해 막연하게만 알고 있었음을 느꼈음. 당연시 여기던 것들이 나름의 원리가 있고 이를 배울 필요를 느낌
